...
public enum OAuthRouter: Router {
    case Authorize(VSOAuthConfiguration)
    case AccessToken(VSOAuthConfiguration, String)
    ...
    public var URLRequest: NSURLRequest? {
        switch self {
        case .Authorize(let config):
            let url = NSURL(string: path, relativeToURL: NSURL(string: config.apiEndpoint))
